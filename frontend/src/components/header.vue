<template>
  <header class="app-header flex items-center justify-between p-4 bg-green-500 text-white">
    ...
    <div class="header-actions flex items-center gap-4">
      <!-- Date picker -->
      <input type="date" v-model="selectedDate" @change="dateChanged" class="rounded px-2 py-1 text-black"/>

      <!-- User profile dropdown -->
      <div class="user-menu relative" @click="toggleMenu">
        <span class="cursor-pointer">{{ user.name }}</span>
        <ul v-if="menuOpen" class="absolute right-0 mt-2 bg-white text-black rounded shadow-md w-48">
...
export default {
  ...
  setup() {
    ...
    const user = inject('user'); // provided from App.vue
