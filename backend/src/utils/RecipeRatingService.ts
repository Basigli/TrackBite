...
export class RecipeRatingService {
   ...
   */
  async rateRecipe(request: RecipeRatingRequest): Promise<RecipeRatingResponse> {
   ...
   */
  private buildPrompt(request: RecipeRatingRequest): string {
    const ingredientsList = request.ingredients
      .map(ing => `- ${ing.name}: Grade ${ing.rating} (Nutrition Score: ${ing.nutritionScore}/100)`)
      .join('\n');
...
Be strict with ratings. Unhealthy cooking methods should significantly impact the final score.`;
   ...
   */
  async rateRecipes(requests: RecipeRatingRequest[]): Promise<RecipeRatingResponse[]> {
    const promises = requests.map(request => this.rateRecipe(request));
...
export { RecipeRatingRequest, RecipeRatingResponse, IngredientRating };